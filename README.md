...
| Pro | 공식 서명·공증 빌드, 안정 업데이트, 여러 기기 등록, 파일 드래그 전송, 원격 연결 서비스 |
...
정확한 드래그 동작, 전송 승인, 충돌 처리와 보안 제한은 별도 설계에서 확정한 뒤 구현합니다.
...
현재는 1단계입니다. 전수검사 이슈 33개 중 15개를 닫았고 18개가 남았습니다.

```mermaid
flowchart LR
  P1["1. v1 안정화<br/>감사 이슈 15/33"] --> P2["2. 공식 배포<br/>서명·공증·안정 업데이트"]
...
| 단계 | 완료 조건 | 현재 상태 |
| --- | --- | --- |
| 1. v1 안정화 | 감사 이슈 #6~#38, 필수 CI와 플랫폼 수용 기준 완료 | **진행 중 — 15/33** |
| 2. 공식 배포 | Windows 서명, Apple 공증, 안정 업데이트와 롤백 | 대기 |
| 3. Pro 기반 | 계정·기기·구독과 파일 드래그 전송 베타 | 설계 시작 |
